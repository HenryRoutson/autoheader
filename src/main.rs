...
fn setup(file_string: &str) {
    ...
    if ext != ".c" && ext != ".h" {
        println!("file didn't contain a c or h extension");
        return;
    }
    ...
    if !Path::new(&h_file_path).exists() {
        println!("h file didn't exist");
        return;
    }
    ...
    File::create(&c_file_path)
        .unwrap()
        .write_all(c_file_content.as_bytes())
        .unwrap();
    File::create(&h_file_path)
        .unwrap()
        .write_all(h_file_content.as_bytes())
        .unwrap();
...
fn create_h(file_string: &str) {
    ...
    if !file_string.ends_with(".c") {
        println!(
            ...
            format!(
                " {} : {} ",
                "doesn't end with .c file extension", file_string
            )
            .on_truecolor(247, 103, 87)
        );
        return;
    } // red
    ...
    let mut c_file_content = String::new();
    File::open(c_file_path)
        .expect("Cannot open file")
        .read_to_string(&mut c_file_content)
        .expect("Error reading file contetnts to string");
    if !c_file_content.contains(PUBLIC_TAG) {
        println!(
            "{}",
            format!(
                " {} : {} ",
                "no public tags, a function prototype file wasn't created", c_file_string
            )
            .on_truecolor(235, 177, 52)
        ); // orange
        return;
    }
    ...
    println!(
        ...
        format!(
            " {} : {} ",
            "functions prototype file was created", h_file_string
        )
        .on_truecolor(135, 245, 166)
    ); // green
    ...
    for s in c_file_content.split(PUBLIC_TAG).skip(1) {
        let function_prototype = s[..s.find('{').expect("{ not found after // public") - 1]
            .trim()
            .as_bytes();
