...
fn check_dir(dir: &PathBuf) {
    ...
    assert!(
        dir.is_dir(),
        "{} : {}",
        dir.to_str().get_or_insert("no valid string"),
        fs::metadata(dir).unwrap_err()
    );
}
...
fn test_convert() {
    ...
    Command::new("cp")
        .arg("convert.c")
        .arg("output/convert.c")
        .output()
        .expect("failed to execute process");
    Command::new("cp")
        .arg("convert.h")
        .arg("output/convert.h")
        .output()
        .expect("failed to execute process");
    ...
    File::open("convert-defs.h")
        .unwrap()
        .read_to_string(&mut defs_content)
        .unwrap();
    File::open("convert.c")
        .unwrap()
        .read_to_string(&mut c_content)
        .unwrap();
